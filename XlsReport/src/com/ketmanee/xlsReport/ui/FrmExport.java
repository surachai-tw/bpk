...
import com.ketmanee.xlsReport.dao.XlsReportDAO;
 ...
 * @author surachai.tw
 */
public class FrmExport extends javax.swing.JFrame
{
    ...
    public FrmExport()
    {
        ...
        XlsReportDAO tmpXlsReportDAO = new XlsReportDAO();
        this.aTxtSqlPath.setText(tmpXlsReportDAO.getDefaultPathSql());
        this.aTxtOutputPath.setText(tmpXlsReportDAO.getDefaultPathOutput());
    ...
    private void initComponents() {
        ...
        aLblOutputPath = new javax.swing.JLabel();
        aTxtOutputPath = new javax.swing.JTextField();
        ...
        aTxtSqlPath.setText("D:\\BPK9\\XlsReport\\sql\\");
            ...
            aLblOutputPath.setText(bundle.getString("FrmExport.aLblOutputPath")); // NOI18N
            gridBagConstraints = new java.awt.GridBagConstraints();
            gridBagConstraints.gridx = 0;
    ...
    private void deleteFileFromFolderWorking()
    {
        File folderWorking = new File("working" + System.getProperty("file.separator"));
        File[] filesWorking = folderWorking.listFiles();
        for (int i = 0; i < filesWorking.length; i++)
        {
            filesWorking[i].delete();
        }
    }

    private void moveFileFromFolderWorkingToOutput(String date)
    {
        try
        {
            ...
            {
                dateFolder = this.aTxtFromDate.getText().trim() + " TO " + this.aTxtToDate.getText().trim();
            }

            File folderOutput = new File(this.aTxtOutputPath.getText().trim() + dateFolder +" "+ this.getCurrentDateTimeForFolder());
            folderOutput.mkdir();
            ...
            for (int i = 0; i < filesWorking.length; i++)
            {
                File fileOutput = new File(folderOutput.getAbsolutePath() + System.getProperty("file.separator") + filesWorking[i].getName());
                filesWorking[i].renameTo(fileOutput);
            }
        } catch (Exception ex)
        {
            ex.printStackTrace();
    ...
    public void execute(String inputFromDate, String inputToDate)
    {
        ...
        this.aTxtOutputPath.setEditable(false);
        ...
        if (!chkPathSql.exists())
        {
            ...
            this.aTxtOutputPath.setEditable(true);
        ...
        // ตรวจสอบ Output Path ต้องกำหนดรูปแบบให้ถูกต้อง
        String pathOutput = this.aTxtOutputPath.getText().trim();
        if (!pathOutput.endsWith("\\"))
        {
            pathOutput = pathOutput + "\\";
        }
        this.aTxtOutputPath.setText(pathOutput);
        File chkPathOutput = new File(pathOutput);
        if (!chkPathOutput.exists())
        {
            this.setCursor(Cursor.getDefaultCursor());
            this.aTxtSqlPath.setEditable(true);
            this.aTxtOutputPath.setEditable(true);
            this.aTxtFromDate.setEditable(true);
            this.aTxtToDate.setEditable(true);
            JOptionPane.showMessageDialog(this, ResourceBundle.getBundle("language").getString("FrmExport.aLblOutputPath") + " " + pathOutput + " doesn't exist", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // ลบ File ออกจาก Folder Working ก่อนดึงข้อมูล
        this.deleteFileFromFolderWorking();

        XlsReportDAO aXlsReportDAO = new XlsReportDAO();
        BufferedReader in = null;
        try
        {
            // aXlsReportDAO.exportData("รายการสั่งตรวจ-รักษา", "select item_code::VARCHAR(255) \"Code\", common_name::VARCHAR(255) \"Item Name\", CAST(replace(COALESCE(get_last_item_price(item_id, '1'), '0'), ',', '') AS FLOAT)::FLOAT \"SALE OPD/ UNIT\", CAST(COALESCE(replace(unit_price_cost, ',', ''), '0') AS FLOAT)::FLOAT \"COST/ UNIT\" FROM item ORDER BY common_name");
            ...
            for (int i = 0; i < listMdl.getSize(); i++)
            {
                ...
                aXlsReportDAO.exportData(fileName.replaceAll(".sql", ""), sqlCmdExe.toString());
            }

            // Move file ไปที่ Output และสร้าง folder ตามวันที่
            this.moveFileFromFolderWorkingToOutput(inputFromDate);
        }
    ...
    public static void main(String args[])
    {
        ...
        java.awt.EventQueue.invokeLater(new Runnable()
        {
            public void run()
            {
                FrmExport aFrmExport = new FrmExport();
                aFrmExport.setVisible(true);

                if(autoRun)
                {
                    aFrmExport.setExecuteDate(FrmExport.getCurrentDateForExecute(-1));
                    aFrmExport.execute();
                    System.exit(1);
                }
            }
    ...
    private javax.swing.JLabel aLblOutputPath;
    ...
    private javax.swing.JTextField aTxtOutputPath;
